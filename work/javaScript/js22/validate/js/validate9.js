  ...
  Validate.prototype = {
    ...
    validateInput(input) {
      ...
      switch (regexObj.type) {
        case "word":
          this.validateType(resultDiv, str, /^\w+$/, this.message.wordErrorMsg, regexObj.successMsg)
          return
        case "number":
          this.validateType(resultDiv, str, /^\d+$/, this.message.numberErrorMsg, regexObj.successMsg)
          return
        case "email":
          this.validateType(resultDiv, str, /^\w+@\w+([.]\w{2,4}){1,2}$/, this.message.emailErrorMsg, regexObj.successMsg)
          return
      }
      //验证该文本框满足不满足规则
      // if (reg.test(str)) {
    ...
    validateRequired(parent) {
      this.showErrorMessage(parent, this.message.requiredErrorMsg)
    },
    validateMinLength(parent) {
      this.showErrorMessage(parent, this.message.minlengthErrorMsg)
    },
    validateMaxLength(parent) {
      this.showErrorMessage(parent, this.message.maxlengthErrorMsg)
    },
