(function () {
  ...
  Validate.prototype = {
    ...
    validateInput(input) {
      ...
      let errorMsg;
      //该对象的字符范围要不要验证
      if (regexObj.minlength !== undefined) {
        errorMsg = this.validateMinLength(str, regexObj)
      }
      //该对象要不要非空验证
      if (regexObj.required === true && !errorMsg) {
        errorMsg = this.validateRequired(str)
      }
      ...
      if (reg.test(str)) {
        ...
        this.showErrorMessage(resultDiv, errorMsg, regexObj.errorMsg)
      }
    },
    validateRequired(value) {
      //只是做一次非空验证 同时返回一个string
      if (value === "") {
        return this.message.requiredErrorMsg
      }
    },
    validateMinLength(value, obj) {
      //文本框,文本框对应的对象
      if (value.length < obj.minlength) {
        return this.message.minlengthErrorMsg
      }
    },
