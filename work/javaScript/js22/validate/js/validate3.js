(function () {
  ...
  var Validate = function (arg1, arg2, arg3) {
    this.form = document.querySelector("#formDiv");
    ...
    this.init(aInput, oSubmitBtn, regex)
  }

  //利用原型,提供动态的方法
  Validate.prototype = {
    constructor: Validate,
    init(aInput, oSubmitBtn, regex) {
      //1.点击按钮的时候 验证表单
      ...
      for (let i = 0; i < aInput.length; i++) {
        aInput[i].addEventListener("input", () => {
          this.validateInput(aInput[i], regex)
        })
        //3.失去某个文本框焦点的时候 也想要验证文本框
        aInput[i].addEventListener("blur", () => {
          this.validateInput(aInput[i], regex)
        })
      }
    },
